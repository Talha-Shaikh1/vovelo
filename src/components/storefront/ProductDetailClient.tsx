...
export function ProductDetailClient({ product, settings }: ProductDetailClientProps) {
  ...
  const handleOrderViaInstagram = async () => {
    ...
    await initiateInstagramOrder({
      ...
      instagramHandle: settings?.instagramHandle || 'vo.velo.eu',
    });
          ...
          <div className="pt-4 border-t border-[#E4E4E0] space-y-3">
            ...
            <div className="flex items-center gap-2.5 px-3.5 py-2.5 bg-[#FAF5FF] border border-[#E9D5FF] rounded-xl text-xs text-[#581C87]">
              ...
              <p className="leading-snug">
                Clicking opens Instagram DM with <strong>@{settings?.instagramHandle || 'vo.velo.eu'}</strong>. Order details & product link will be copied automatically so you can paste & send.
              </p>
            </div>
          </div>
